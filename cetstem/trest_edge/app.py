...
app.config["JSON_AS_ASCII"] = False
WorkModulePath = '/app/appexample/trest/code.py'
TaskResult = namedtuple('TaskResult', ['code', 'result', 'task', 'node', 'taskname'])
...
def manage(path):
    if path:
        ...
        if recv_data:
            ...
            taskname = recv_data.get('taskname')
            taskres = TaskResult(code=code, result=None, task=task, node=node, taskname=taskname)
            cache.set('tasktuple', taskres, timeout=800)
            if task:
